...
def parse_vcf_line(line, annovar=False):
    CHROM = 0
    POS = 1
    ID = 2
    REF = 3
    ...
    # there's a weird bug in annovar that's outputting a column of "."
    if annovar is True:
        del fields[0]
...
def is_eQTLpos(vcf_info):
    # Check CYP2D6_eQTLs
    if "eQTLpos" in vcf_info:
        eQTL = vcf_info["eQTLpos"]
        if eQTL != ".":
            return True
    return False

def is_eQTLneg(vcf_info):
    # Check CYP2D6_eQTLs
    if "eQTLneg" in vcf_info:
        eQTL = vcf_info["eQTLneg"]
        if eQTL != ".":
            return True
    return False
...
def variant_class(vcf_info):
    # get the variant class for refseq
    refseq_func = ""
    if "ExonicFunc.refGene" in vcf_info:
        refseq_func = vcf_info["ExonicFunc.refGene"]
    ...
    if "CSQ" in vcf_info:
        ...
        vep_annotations = [dict(zip(vep_fields, x.split('|'))) for x in vcf_info['CSQ'].split(',')]



        for transcript in vep_annotations:
            if not transcript['SYMBOL'] in ["CFTR", "CYP2D6", "CYP2C9", "CYP2C19", "IFNL3", "SLCO1B1", "UGT1A1", "CYP2B6", "CYP4F2",
                        "TPMT", "DPYD", "VKORC1", 'NUDT15','CYP3A5']:
                continue
            gene = transcript['SYMBOL']
            vep_func.add(transcript['Consequence'])


    return ",".join(vep_func)

def get_gene(vcf_info):
        gene = None
        ...
        if "CSQ" in vcf_info:
            ...
            for transcript in vep_annotations:
                if not transcript['SYMBOL'] in ["CFTR", "CYP2D6", "CYP2C9", "CYP2C19", "IFNL3", "SLCO1B1", "UGT1A1",
                                              "CYP2B6", "CYP4F2",
                                              "TPMT", "DPYD", "VKORC1", 'NUDT15', 'CYP3A5']:
                    continue
                gene = transcript['SYMBOL']
...
def get_impact(vcf_info):
    variant_impact = variant_class(vcf_info).split(",")

    high_impact = ['transcript_ablation', 'splice_acceptor_variant', 'splice_donor_variant', 'stop_gained',
                   'frameshift_variant', 'stop_lost', 'start_lost', 'transcript_amplification']

    for i in variant_impact:
        for h in high_impact:
            if h in i:
                return h

    # moderate
    mod_impact = ['missense_variant', 'inframe_insertion', 'inframe_deletion', 'protein_altering_variant', 'regulatory_region_ablation']
    ...
    # moderate
    low_impact = ['splice_region_variant', 'incomplete_terminal_codon_variant', 'start_retained_variant', 'stop_retained_variant',
                  'synonymous_variant']
